...
- `SITE_URL`: `https://toolscastle.app` (or your temporary `*.pages.dev` URL).
- `BASE_PATH`: `/`
- `NODE_VERSION`: `20`
...
6. Add Environment Variable: `SITE_URL` = `https://toolscastle.app`.
7. Click **Deploy**.
...
3. Enter your domain: `toolscastle.app` (and repeat for `www.toolscastle.app`).
...
5. If your domain is registered on Namecheap, GoDaddy, or Google Domains:
   - Add a `CNAME` record pointing `toolscastle.app` (or `@`) to `<project-name>.pages.dev`.
...
2. Click **Add Property** → Choose **URL prefix** (e.g. `https://toolscastle.app/`).
...
2. Click **Sites** → **Add Site** → Enter `https://toolscastle.app`.
